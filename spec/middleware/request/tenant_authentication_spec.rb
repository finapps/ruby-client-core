...
RSpec.describe FinAppsCore::Middleware::TenantAuthentication do
  ...
  describe '#call' do
    ...
    context 'when company credentials were provided' do
      let(:middleware) { described_class.new(fake_app, VALID_CREDENTIALS[:token]) }
      ...
      context 'when header was previously set' do
        ...
        it('does not generate a Tenant Authentication header') { expect(actual_header).not_to eq(expected_header) }
      end
    end
  end
end
